      ...
      <nav className='flex items-center h-full justify-between w-[90%] xl:w-[80%] mx-auto'>
        ...
        <div className='flex items-center space-x-2'>
          <div className='h-10 w-10 bg-blue-950 rounded-full flex items-center justify-center'>
            <MdDeliveryDining className='text-white h-6 w-6' />
          </div>
          <span className='text-xl font-bold hidden sm:block md:text-2xl text-blue-950'>
            Foodie
          </span>
        </div>
        {/* NAVLINKS */}
          ...
          {NavLinks.map((link) => {
            return (
              <Link
                key={link.id}
                href={link.url}
                className='text-black hover:text-green-700 font-bold transition-all duration-200 hover:border-b-2'
              >
                <li>{link.label}</li>
              </Link>
            );
        ...
        <div className='flex items-center space-x-4'>
          ...
          <HiBars3BottomRight
            onClick={openNav}
            className='w-8 h-8 text-blue-950 cursor-pointer lg:hidden'
          />
        </div>
      </nav>
    </header>
